...
19 APR 2012		Justin Crooks		Updated for Windows-based DLL
...
{
public:
	__declspec(dllimport) CAudioStream(void);
	__declspec(dllimport) ~CAudioStream(void);

	__declspec(dllimport) int ReadAudioChunk(short * buffer);
	__declspec(dllimport) void StopAudio();
	__declspec(dllimport) void StartAudio();
	void * m_pvUSB;
